...
N_enc_default = int(10**7)
N_enc_max = int( np.min([N_enc, N_enc_default]) )
N_timestep_max = int(N_enc_max / K)
N_blocks = int( np.ceil( N / N_timestep_max) ) 
N_timesteps_in_block_list = N_timestep_max * np.ones(N_blocks, dtype = int)
N_timesteps_in_block_list[-1] = N - (N_timestep_max * (N_blocks - 1) )
N_enc_in_block_list = K * N_timesteps_in_block_list


N_enc_in_block = N_enc_in_block_list[0]
N_timesteps_in_block = N_timesteps_in_block_list[0]

p_sample = np.sqrt(np.random.random(size = N_enc_in_block)) * R     # PDF: 2p
theta_sample = np.arccos(2 * ( np.random.random(size = N_enc_in_block) ) - np.ones(N_enc_in_block))   # PDF: sin \theta / 2
phi_sample = np.random.random(size = N_enc_in_block) * 2 * np.pi    # PDF: Uniform [0,2\pi)
gamma_sample = np.random.random(size = N_enc_in_block) * 2 * np.pi    # PDF: Uniform [0,2\pi)
v_sample = v_rel_sampler(v0,vesc,N_enc_in_block)    # PDF: Maxwellian with velocity dispersion v0 [0,\infty)
tau_rand = np.random.random(size = N_enc_in_block) # PDF for effective observation time
...
p_sample=p_sample.reshape(-1,N_timesteps_in_block)
theta_sample=theta_sample.reshape(-1,N_timesteps_in_block)
phi_sample=phi_sample.reshape(-1,N_timesteps_in_block)
v_sample=v_sample.reshape(-1,N_timesteps_in_block)
gamma_sample=gamma_sample.reshape(-1,N_timesteps_in_block)
tau_rand=tau_rand.reshape(-1,N_timesteps_in_block)
...
M_data_no_nan = M_data[~np.isnan(M_data)]
rand_ind = np.random.randint(0, high=len(M_data_no_nan)-1, size=K, dtype=int)
M_sample = np.array([M_data_no_nan[n] for n in rand_ind])
#M_sample = np.ones(K)
#mu_sample = q / (1 + q)**2 * M
...
for i_block in range(1,N_blocks):
    N_enc_in_block = N_enc_in_block_list[i_block]
    N_timesteps_in_block = N_timesteps_in_block_list[i_block]
    ...
    for timestep in range(0,p_sample.shape[1]):
        ### observation time occurs at random time in between encounter
        tau_sample_obs = tau_sample + tau_rand[:,timestep] * delta_t / period_(a_sample,k_sample)
...
#input_vec = [a0,log_m,log_rs,log_c,log_N_enc_approx]
